...
public class FindKthLargest {
    ...
    public static int findKthLargest(int[] arr, int k) {
        return process(arr, 0, arr.length - 1, arr.length - k, new Random());
    }

    private static int process(int[] arr, int l, int r, int k, Random random) {
        int p = partition(arr, l, r, random);

        if (p == k) return arr[p];

        if (p < k) return process(arr, p + 1, r, k, random);
        return process(arr, l, p - 1, k, random);
    }
